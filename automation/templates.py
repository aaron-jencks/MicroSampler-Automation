...
def template_insert_file(ctx: Dict, client: TemplateController, tag_name: str, args: List[str]) -> str:
    if len(args) < 1:
...
def template_insert_template(ctx: Dict, client: TemplateController, tag_name: str, args: List[str]) -> str:
    if len(args) < 1:
        raise RuntimeError(f"Expected at least one argument, got {len(args)}")
...
def template_insert_config_value(ctx: Dict, client: TemplateController, tag_name: str, args: List[str]) -> str:
    if len(args) < 1:
        raise RuntimeError(f"Expected at least one argument, got {len(args)}")
    current = ctx
    for key in args:
...
def template_insert_allowed_references(ctx: Dict, client: TemplateController, tag_name: str, args: List[str]) -> str:
    result = []
